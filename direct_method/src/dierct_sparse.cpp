...
int main(int argc, char* argv[])
{
    ...
    for(int index = 0; index < 10; index++)
    {
        ...
        if(index == 0)
        {
            ...
            {
                //remove edge point
                if(p.pt.x < 20 || p.pt.y < 20 || ( p.pt.x+20 ) >color.cols || ( p.pt.y+20 ) >color.rows)
                    continue;
                ushort d = depth.ptr<ushort> (cvRound(p.pt.y))[cvRound(p.pt.x)];
                if (d == 0)
                    continue;
                
                Eigen::Vector3d p3d = project2Dto3D(p.pt.x, p.pt.y, d, fx, fy, cx, cy, depth_scale);
                float grayscale = float(gray.ptr<uchar>(cvRound(p.pt.y))[cvRound(p.pt.x)]);
        ...
        std::cout << "sparse direct method costs time: " << time_used.count() << " second" << std::endl;
